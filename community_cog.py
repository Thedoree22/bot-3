...
class CommunityCog(commands.Cog):
    ...
    async def create_join_leave_image(self, member_name: str, guild_name: str, avatar_url: Optional[str], mode: str) -> Optional[discord.File]:
        try:
            ...
            if mode == "join":
                line1_text = "მოგესალმებით"; line3_text = f"{guild_name} - ში!"; name_color = (255, 255, 255)
            else: # mode == "leave"
                line1_text = "დაგვტოვა"; line3_text = f"{guild_name} - დან"; name_color = (200, 200, 200)
...
async def setup(bot: commands.Bot):
    if isinstance(bot, commands.Bot):
        ...
        print("შეცდომა: CommunityCog არასწორი bot ობიექტი setup-ში")
